...
public class MembroController implements Loggable {
	...
	public Page<Membro> obterTodosMembros(Pageable pageable) {
		return service.findAll(pageable);
	}

	@RequestMapping(method = RequestMethod.GET, path = "/{id}", produces = "application/json")
	@ApiOperation("Retorna um membro específico pelo seu identificador. 404 se não for encontrado.")
	public ResponseEntity<CadastroBasicoDTO> obterMembroPorId(
			@ApiParam("Identificador do membro. Não pode ser vazio.") @PathVariable long id) {
		Optional<CadastroBasicoDTO> cadastroBasico = service.findBy(id);

		if (cadastroBasico.isPresent()) {
			return ResponseEntity.ok(cadastroBasico.get());
		}
	...
	public ResponseEntity<Membro> excluirMembro(
			@ApiParam("Identificador do membro a ser deletado. Não pode ser vazio.") @PathVariable long id) {
		Optional<Membro> membro = service.findById(id);

		if (membro.isPresent()) {
			service.delete(membro.get());
	...
	public ResponseEntity<CadastroBasicoDTO> criarCadastroBasico(
			@ApiParam("Novos dados básicos de um membro.") @RequestBody CadastroBasicoDTO cadastroBasico) {
		try {
